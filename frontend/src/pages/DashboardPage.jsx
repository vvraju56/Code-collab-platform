    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-800/60 bg-slate-950/90 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={logout}
              className="px-4 py-2 text-xs font-black text-slate-400 hover:text-white uppercase tracking-widest transition border border-slate-800 hover:border-slate-600 rounded-xl"
            >
              Sign Out
            </button>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <p className="text-slate-400 text-lg">Your collaborative workspace — all in one place.</p>
        </motion.div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white font-black text-sm uppercase tracking-widest rounded-2xl transition shadow-xl shadow-blue-900/40 hover:scale-105 active:scale-100"
              >
            ...
            {loading ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                {[1,2,3].map(i => (
                  <div key={i} className="h-52 rounded-2xl bg-slate-900 animate-pulse border border-slate-800" />
                ))}
              </div>
            ) : filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                <div className="text-6xl mb-4">📁</div>
                <p className="text-2xl font-black text-slate-600 tracking-tighter">No projects yet</p>
                <p className="text-slate-600 mt-2 mb-6">Create your first project to start collaborating</p>
                <button onClick={() => setShowCreate(true)} className="px-6 py-3 bg-blue-600 text-white font-black text-sm rounded-2xl hover:bg-blue-500 transition">
                  Create Project
                </button>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                {filtered.map((project, i) => (
                  <motion.div
                    ...
                    whileHover={{ y: -4 }}
                    className="group relative p-6 rounded-2xl border border-slate-800 bg-slate-900/60 hover:bg-slate-900 cursor-pointer transition-all hover:border-slate-700 hover:shadow-2xl hover:shadow-slate-900"
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
                        <button
                          onClick={e => { e.stopPropagation(); if (confirm("Delete this project?")) deleteProject(project._id); }}
                    ...
                    <h3 className="font-black text-white text-lg tracking-tight mb-1 truncate">{project.name}</h3>
                    <p className="text-slate-500 text-sm mb-4 line-clamp-2">{project.description || "No description"}</p>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {project.isPublic && <span className="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-green-500/10 text-green-400">Public</span>}
                      </div>
        ...
        {activeTab === "members" && (
          <div className="space-y-6">
            {/* Header */}
            <div
              className="relative rounded-3xl overflow-hidden p-10 flex flex-col md:flex-row md:items-center justify-between gap-6"
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h2 className="text-4xl font-black text-white tracking-tighter mb-1">Active Team</h2>
                <p className="text-slate-400 text-sm">{activeMembers.length} developer{activeMembers.length !== 1 ? "s" : ""} connected right now.</p>
              </div>
              ...
              <div className="relative z-10 flex gap-4">
                {[
                  ...
                  <div key={s.label} className="text-center px-8 py-5 rounded-2xl"
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="font-black text-white text-lg uppercase tracking-tighter truncate leading-none mb-1">
                      ...
                      >
                        ...
                        <span className="ml-auto text-[9px] font-bold text-green-700">●●●</span>
                      </div>
                    </div>
                  </motion.div>
                );
              ...
              {activeMembers.length === 0 && (
                <div className="col-span-3 flex flex-col items-center justify-center py-20 text-center"
                  style={{ background: "linear-gradient(135deg,#080616,#0f0a2e)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "1.5rem" }}>
                  <div className="text-5xl mb-4">🔍</div>
                  <p className="text-xl font-black text-slate-500 uppercase tracking-tighter">Scanning for peers...</p>
                  <p className="text-xs font-bold text-slate-700 uppercase tracking-widest mt-2">No active developers on this node</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === "stats" && (
          <div className="grid md:grid-cols-2 gap-6">
            <div className="p-8 rounded-2xl border border-slate-800 bg-slate-900/60">
              <h3 className="text-lg font-black uppercase tracking-widest mb-6 text-white">Your Activity</h3>
            ...
            <div className="p-8 rounded-2xl border border-blue-500/20 bg-blue-600/10">
              <h3 className="text-lg font-black uppercase tracking-widest mb-2 text-white">Platform</h3>
              <p className="text-6xl font-black text-white tracking-tighter mb-2">{stats.totalUsers}</p>
              <p className="text-xs font-black uppercase tracking-[0.3em] text-blue-300/60">Registered Developers</p>
              ...
              <form onSubmit={handleCreateProject} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3.5 bg-slate-800 border border-slate-700 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 font-medium"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3.5 bg-slate-800 border border-slate-700 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 font-medium resize-none"
                  />
                </div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors ${newProject.isPublic ? "bg-blue-600" : "bg-slate-700"}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${newProject.isPublic ? "left-7" : "left-1"}`} />
                  </button>
                  <span className="text-sm font-semibold text-slate-300">Public project</span>
                </div>

                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setShowCreate(false)}
                    className="flex-1 py-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-black rounded-2xl transition text-sm uppercase tracking-widest">
                    Cancel
                  </button>
                  <button type="submit" disabled={creating}
                    className="flex-1 py-3.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-black rounded-2xl transition text-sm uppercase tracking-widest shadow-xl shadow-blue-900/40">
