...
// GET /api/projects/join-requests - get all join requests for user's projects
router.get("/join-requests", async (req, res) => {
  try {
    ...
    const result = projects
      .filter(p => p.joinRequests?.length > 0)
      .map(p => ({
        _id: p._id,
        name: p.name,
        joinRequests: p.joinRequests.filter(r => r.status === "pending")
      }));
...
router.post("/", async (req, res) => {
  try {
    ...
    const existingProject = await Project.findOne({ name: name.trim(), owner: req.user._id });
    if (existingProject) {
